...
/**
 * Calculates spatial autocorrelation for given spatial data.
 *
...
export function spatialAutocorrelation(spatialData, valueProperty, options={}) {
  ...
  if (LOCAL_METHOD_SET.has(method)) {
    ...
    if (!outputStatisticStatistics) {
      results.forEach(result => {
        delete result.expectedStatistic
        delete result.statisticMean
        delete result.statisticStd
      })
    }

    if (!outputCutoffs) {
      results.forEach(result => {
        delete result.lowerCutoff
        delete result.upperCutoff
      })
    }
